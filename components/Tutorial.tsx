...
const Tutorial: React.FC<TutorialProps> = ({ onComplete, onSkip }) => {
  ...
  const playSound = (type: 'pop' | 'success' | 'hover') => {
    try {
  ...
  const handleDragMove = (e: React.MouseEvent) => {
    if (isDragging && containerRef.current) {
      ...
      // Check if near target (the mouse 🐭)
  ...
  return (
    ...
    >
      <style>{`
        ...
        .star-pop {
          animation: star-pop-bounce 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards;
        }
      `}</style>
