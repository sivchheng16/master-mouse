...
export const RicePlanting: React.FC<{ onComplete: () => void; count?: number }> = ({ onComplete, count = 6 }) => {
    ...
    const handleMouseUp = () => {
        ...
        if (targetPlot) {
            ...
            setPlots(prev => prev.map(p => p.id === targetPlot.id ? { ...p, filled: true } : p));
            setSeedlings(prev => prev.map(s => s.id === dragging ? { ...s, planted: true } : s));
    ...
    return (
        ...
        >
            ...
            {/* Seedlings to drag */}
            {seedlings.filter(s => !s.planted).map(seedling => (
                <div
                    key={`seedling-${seedling.id}`}
                    className={`absolute cursor-grab active:cursor-grabbing transition-transform ${dragging === seedling.id ? 'scale-125 z-50 opacity-75' : 'hover:scale-110'
                        }`}
                    style={{
                        left: dragging === seedling.id ? `${dragPos.x}%` : `${seedling.x}%`,
                        top: dragging === seedling.id ? `${dragPos.y}%` : `${seedling.y}%`,
                        transform: 'translate(-50%, -50%)',
                    }}
                    onMouseDown={(e) => {
                        e.preventDefault();
                ...
                >
                    <div className="text-4xl md:text-5xl drop-shadow-lg hover:animate-wiggle">🌾</div>
                </div>
            ))}
            ...
            {showLevelUp && (
                <div className="absolute inset-0 flex items-center justify-center bg-amber-900/40 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
                    <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-8 border-amber-200 text-center">
                        <h2 className="title-font text-5xl text-amber-600 animate-bounce mb-4 uppercase">អស្ចារ្យ!</h2>
                        <p className="text-xl font-black text-amber-900">ស្រែកាន់តែធំ! 🌾🌱</p>
                    </div>
                </div>
            )}
            ...
            {round === totalRounds && planted === currentCount && !showLevelUp && (
                <div className="absolute inset-0 flex items-center justify-center bg-amber-900/20 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
                    <div className="bg-white/90 p-12 rounded-[3.5rem] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.4)] border-8 border-white text-center">
                        <h2 className="title-font text-5xl text-green-600 animate-bounce drop-shadow-lg mb-6">ដាំស្រូវរួចហើយ! 🎉</h2>
