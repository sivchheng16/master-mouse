...
export const WaterSplash: React.FC<{ onComplete: () => void; count?: number }> = ({ onComplete, count = 5 }) => {
    ...
    // Decorative elements
    const decorations = useMemo(() => Array.from({ length: 8 }).map((_, i) => ({
        id: i,
        x: 5 + Math.random() * 90,
        y: 10 + Math.random() * 80,
        emoji: ['🎉', '🎊', '🎋', '🌸', '🌺', '☀️', '🦋', '🐦'][i],
    ...
    return (
        <div className="relative w-full h-full overflow-hidden select-none bg-gradient-to-b from-sky-400 via-sky-300 to-green-400">
            ...
            {people.map(person => (
                <div
                    key={person.id}
                    className={`absolute transition-all duration-300 ${person.splashed ? 'scale-110' : 'cursor-pointer hover:scale-110'
                        }`}
                    style={{
                        left: `${person.x}%`,
                        top: `${person.y}%`,
                    ...
                    onClick={() => !person.splashed && handleSplash(person.id, person.x, person.y)}
                    onMouseEnter={() => !person.splashed && audioService.playHover()}
                >
                    <div className={`text-5xl md:text-6xl ${person.splashed ? 'animate-wiggle' : 'animate-bounce-slow'}`}>
            ...
            <div className="absolute bottom-8 right-8 text-6xl animate-bounce-slow" style={{ animationDelay: '0.5s' }}>🪣</div>
            ...
            {showLevelUp && (
                <div className="absolute inset-0 flex items-center justify-center bg-sky-900/40 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
                    <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-8 border-sky-200 text-center">
                        <h2 className="title-font text-5xl text-sky-600 animate-bounce mb-4">សប្បាយណាស់!</h2>
                        <p className="text-xl font-black text-sky-900">មានមនុស្សច្រើនជាង! 💦</p>
                    </div>
                </div>
            )}
            ...
            {round === totalRounds && splashed === currentCount && !showLevelUp && (
                <div className="absolute inset-0 flex items-center justify-center bg-sky-900/20 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
                    <div className="bg-white/90 p-12 rounded-[3.5rem] shadow-2xl border-8 border-white text-center">
                        <h2 className="title-font text-5xl text-sky-600 animate-bounce mb-6">សួស្តីឆ្នាំថ្មី! 🎉</h2>
