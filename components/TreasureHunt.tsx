...
export const TreasureHunt: React.FC<TreasureHuntProps> = ({ onComplete }) => {
  ...
  return (
    ...
    >
      ...
      {/* Instructional HUD - Scroll Style */}
      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-30 bg-[#fdf5e6] px-8 py-3 rounded-md border-b-4 border-r-4 border-[#8b5e34] shadow-2xl text-[#5d4037] text-center min-w-[320px] transform rotate-[-1deg]">
        <div className="absolute -left-2 top-0 bottom-0 w-4 bg-[#d2b48c] rounded-full shadow-inner" />
        <div className="absolute -right-2 top-0 bottom-0 w-4 bg-[#d2b48c] rounded-full shadow-inner" />
        <h2 className="title-font text-xl md:text-2xl mb-1 border-b border-[#8b5e34]/30 pb-1">ស្វែងរកកំណប់ទាំង ៣! ({foundCount}/3)</h2>
        <p className="text-[10px] md:text-xs font-black uppercase tracking-widest text-[#8b5e34]">
          {foundCount === targets.length 
            ? 'កូនរកឃើញកំណប់ទាំងអស់ហើយ! 🎉' 
            : currentProgress < 0.9 
              ? 'រំកិលម៉ៅតាមគន្លងពណ៌ក្រហម...' 
              : 'ចុច ២ដងលើ "X" ដើម្បីជីកកំណប់!'}
        </p>
      </div>
