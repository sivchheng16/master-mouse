  ...
  return (
    ...
    >
      <GameHUD
        ...
        instruction="គូសប្រអប់ដើម្បីចាប់ផ្កាយ!"
        score={caughtCount}
        goal={currentCount}
        actionType="Drag"
      />
      ...
      {showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-indigo-950/60 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-8 border-indigo-200 text-center transform scale-125">
            ...
            <p className="text-xl font-black text-indigo-900">នៅមានផ្កាយច្រើនទៀត... ⭐✨ </p>
          </div>
        </div>
      )}
      ...
      {isGameComplete && (
        <div className="absolute inset-0 flex items-center justify-center bg-indigo-950/60 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-8 border-indigo-200 text-center transform scale-125">
            <h2 className="title-font text-5xl text-indigo-600 animate-bounce mb-4 uppercase">ចាប់បានអស់ហើយ! 🎉</h2>
            <p className="text-xl font-black text-indigo-900">ពូកែណាស់កូន! 🌟</p>
          </div>
        </div>
      )}
    </div>
