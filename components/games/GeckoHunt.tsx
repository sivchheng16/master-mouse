...
export const GeckoHunt: React.FC<{
  onComplete: () => void;
  count?: number;
}> = ({ onComplete, count = 6 }) => {
  ...
  const initRound = (r: number) => {
    ...
    const newGeckos: Gecko[] = Array.from({ length: numGeckos }).map(
      (_, i) => ({
        id: i,
        x: 10 + Math.random() * 80,
        y: 20 + Math.random() * 60,
    ...
    <div className="relative w-full h-full overflow-hidden select-none bg-[#e8e6e1]">
      ...
      {wallElements.map((el) => (
        <div
          ...
          className={`absolute pointer-events-none opacity-40 mix-blend-multiply ${
            el.type === "crack" ? "text-stone-700/60" : "text-stone-500/40"
          }`}
          style={{
            left: `${el.x}%`,
            top: `${el.y}%`,
            transform: `scale(${1 + Math.random()}) rotate(${Math.random() * 360}deg)`,
          }}
        >
      ...
      {geckos.map((gecko) => (
        <div
          ...
          onClick={() =>
            gecko.visible && !gecko.caught && handleCatch(gecko.id)
          }
          onMouseEnter={() =>
            gecko.visible && !gecko.caught && audioService.playHover()
          }
        >
          ...
          {/* The Gecko itself */}
          <div
            className={`relative text-5xl md:text-6xl drop-shadow-md filter brightness-110 ${
              gecko.visible && !gecko.caught ? "animate-gecko" : ""
            }`}
          >
            🦎
          </div>
      ...
      {showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-[100] animate-in fade-in zoom-in duration-500">
          <div className="bg-[#f0fdf4] p-12 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-8 border-green-400 text-center transform hover:scale-105 transition-transform">
            <h2 className="title-font text-6xl text-green-600 animate-bounce mb-4 drop-shadow-sm">
              ពូកែណាស់​!
            </h2>
            <p className="text-2xl font-black text-stone-700 opacity-90">
              ជីងចក់ចេញមកកាន់តែលឿន! 🦎💨
            </p>
          </div>
        </div>
      )}
      ...
      {round === totalRounds && caught === currentCount && !showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-[0_0_100px_rgba(255,255,255,0.5)] border-8 border-[#58cc02] text-center">
            <h2 className="title-font text-6xl text-transparent bg-clip-text bg-gradient-to-r from-green-500 to-emerald-600 animate-bounce mb-8">
              ចាប់គ្រប់អស់ហើយ! 🎉
            </h2>
