...
export const BalloonSizer: React.FC<BalloonSizerProps> = ({
  onComplete,
  tolerance = 12,
}) => {
  ...
  return (
    ...
    >
      <GameHUD
        instruction={languageService.t("game.balloon_sizer_instruction")}
      />
      ...
      {!success && (
        <div className="absolute left-10 top-1/2 -translate-y-1/2 flex flex-col items-center gap-4 z-20 opacity-60">
          <div className="w-10 h-16 border-4 border-sky-400 rounded-2xl relative flex justify-center p-1">
            <div
              className={`w-2 h-4 bg-sky-500 rounded-full ${isScrolling ? "animate-bounce" : "animate-pulse"}`}
            />
          </div>
          <span className="text-[10px] font-black text-sky-800 uppercase tracking-widest vertical-text">
            Scroll Up/Down
          </span>
        </div>
      )}
      ...
      <div className="relative flex items-center justify-center pointer-events-none">
        ...
        <div
          className={`absolute border-[6px] border-dashed rounded-full transition-all duration-300 flex items-center justify-center ${
            isPerfect
        ...
        >
          <div
            className={`w-full h-full rounded-full transition-colors duration-300 shadow-2xl relative flex items-center justify-center ${
              isPerfect
                ? "bg-emerald-500"
                : isNear
                  ? "bg-orange-400"
            ...
            style={{
              ...
              boxShadow:
                "inset -5px -10px 15px rgba(0,0,0,0.1), 0 20px 40px rgba(0,0,0,0.15)",
            }}
          >
            <span className="text-white/20 text-3xl md:text-5xl font-black">
              🎈
            </span>
            <div
          ...
          <svg
            className="absolute top-[98%] left-1/2 -translate-x-1/2 w-10 h-32 overflow-visible opacity-30"
            viewBox="0 0 20 100"
          >
            <path
      ...
      <div className="absolute bottom-16 w-full max-w-sm px-8 z-10">
        <div className="bg-white/40 backdrop-blur-sm rounded-full h-3 border border-white/40 shadow-inner overflow-hidden flex items-center p-0.5">
          <div
            ...
            style={{
              width: `${Math.max(5, Math.min(100, (1 - diff / 200) * 100))}%`,
            }}
          />
        </div>
      </div>
      ...
      {success && (
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-emerald-600 font-black text-2xl md:text-3xl animate-bounce bg-white/90 px-10 py-4 rounded-full border-4 border-emerald-100 shadow-2xl z-50">
          {languageService.t("game.well_done")} 🏆✨
        </div>
      )}
