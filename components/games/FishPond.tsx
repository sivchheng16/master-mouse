...
export const FishPond: React.FC<{ onComplete: () => void; count?: number }> = ({
  onComplete,
  count = 5,
}) => {
  ...
  const handleFeedFish = (fishId: number) => {
    ...
    setFishes((prev) =>
      prev.map((f) => (f.id === fishId ? { ...f, fed: true } : f)),
    );
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(180deg, #2d5016 0%, #1a3009 20%, #0a3d62 50%, #0a2d4e 100%)",
      }}
    >
      {/* Lily pads */}
      {[...Array(6)].map((_, i) => (
      ...
      <GameHUD
        ...
        instruction="ផ្ដិតម៉ៅដើម្បីបំបាក់ត្រី! 🐟"
        score={fed}
        goal={currentCount}
      />
      ...
      {fishes.map((fish) => (
        <div
          ...
          className={`absolute transition-all duration-300 ${
            fish.fed ? "opacity-50 scale-125" : "cursor-pointer hover:scale-110"
          }`}
          style={{
            left: `${fish.x}%`,
            top: `${fish.y}%`,
        ...
        >
          <div
            className={`text-4xl md:text-5xl drop-shadow-xl ${
              fish.attracted && !fish.fed ? "animate-wiggle" : "animate-swim"
            }`}
          >
            {fish.emoji}
          </div>
      ...
      {round === totalRounds && fed === currentCount && !showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-blue-900/20 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
          <div className="bg-white/90 p-12 rounded-[3.5rem] shadow-2xl border-8 border-white text-center">
            <h2 className="title-font text-5xl text-blue-600 animate-bounce mb-6">
              ត្រីឆ្អែតហើយ! 🎉
            </h2>
