...
export const PatternMaster: React.FC<{
  onComplete: () => void;
  length?: number;
  speed?: number;
}> = ({ onComplete, length = 3, speed = 800 }) => {
  ...
  const initRound = (r: number) => {
    ...
    const newSeq = Array.from({ length: currentLength }).map(() =>
      ...
      <div className="grid grid-cols-2 gap-4 md:gap-8">
        {[
          ...
          <button
            ...
            className={`w-24 h-24 md:w-36 md:h-36 text-5xl md:text-7xl rounded-[2rem] md:rounded-[3rem] transition-all duration-300 transform border-4 shadow-xl ${
              active === i
      ...
      {showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-rose-900/40 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-8 border-rose-100 text-center">
            <h2 className="title-font text-5xl text-rose-600 animate-bounce mb-4 uppercase">
              អស្ចារ្យ!
            </h2>
