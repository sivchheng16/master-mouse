...
export const LotusBloom: React.FC<{
  onComplete: () => void;
  count?: number;
}> = ({ onComplete, count = 5 }) => {
  ...
  const initRound = (r: number) => {
    ...
    const newLotuses: Lotus[] = Array.from({ length: numLotuses }).map(
      (_, i) => ({
        id: i,
        x: 15 + Math.random() * 70,
        y: 35 + Math.random() * 45,
  ...
  const handleClick = (lotusId: number) => {
    ...
    if (lotus.stage === "flower") {
      ...
      setLotuses((prev) =>
        prev.map((l) => (l.id === lotusId ? { ...l, bloomed: true } : l)),
      );
  ...
  return (
    <div className="relative w-full h-full overflow-hidden select-none bg-gradient-to-b from-sky-300 via-sky-400 to-teal-700">
      ...
      <div
        className="absolute inset-0 bg-transparent opacity-30 pointer-events-none"
        style={{
          backgroundImage:
            "radial-gradient(circle, rgba(255,255,255,0.2) 2px, transparent 10px)",
      ...
      {lotuses.map((lotus) => (
        <div
          ...
          className={`absolute cursor-pointer transition-all duration-200 z-20 ${
            lotus.bloomed ? "scale-125" : "hover:scale-110"
          }`}
          style={{
            left: `${lotus.x}%`,
            top: `${lotus.y}%`,
        ...
        >
          ...
          <div
            className={`text-5xl md:text-6xl drop-shadow-2xl transition-all duration-300 ${
              lotus.stage === "flower" && !lotus.bloomed
      ...
      <div className="absolute top-[30%] left-[20%] text-3xl animate-dragonfly pointer-events-none z-30">
        🦋
      </div>
      <div
        className="absolute top-[40%] right-[25%] text-2xl animate-dragonfly pointer-events-none z-30"
      ...
      {showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-pink-900/40 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-8 border-pink-200 text-center">
            <h2 className="title-font text-5xl text-pink-600 animate-bounce mb-4">
              ផ្កាស្រស់ស្អាតណាស់!
            </h2>
      ...
      {round === totalRounds && bloomed === currentCount && !showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-pink-900/20 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
          <div className="bg-white/90 p-12 rounded-[3.5rem] shadow-2xl border-8 border-white text-center">
            <h2 className="title-font text-5xl text-pink-600 animate-bounce mb-6">
              ផ្កាឈូករីកអស់ហើយ! 🎉
            </h2>
