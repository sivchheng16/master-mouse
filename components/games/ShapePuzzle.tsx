...
export const ShapePuzzle: React.FC<{ onComplete: () => void }> = ({
  onComplete,
}) => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    setShapes((prev) =>
      prev.map((s) =>
        s.id === draggingId ? { ...s, currentX: x, currentY: y } : s,
      ),
    );
  };
  ...
  const handleMouseUp = () => {
    ...
    setShapes((prev) => {
      ...
      const dist = Math.sqrt(
        Math.pow(shape.currentX - shape.targetX, 2) +
          Math.pow(shape.currentY - shape.targetY, 2),
      );

      let nextShapes;
      if (dist < 10) {
        audioService.playCollect();
        nextShapes = prev.map((s) =>
          s.id === draggingId
            ? { ...s, isPlaced: true, currentX: s.targetX, currentY: s.targetY }
            : s,
        );
      } else {
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        {shapes.map((s) => (
          ...
          >
            <span className="text-4xl md:text-6xl opacity-10 grayscale">
              {s.emoji}
            </span>
          </div>
        ))}
      ...
      {shapes.map((s) => (
        <div
          ...
          className={`absolute w-24 h-24 md:w-32 md:h-32 rounded-3xl flex items-center justify-center text-5xl md:text-7xl shadow-lg border-4 transition-all ${
            s.isPlaced
      ...
      {shapes.every((s) => s.isPlaced) && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/40 backdrop-blur-md z-30 animate-in zoom-in duration-500">
          <div className="bg-white p-12 rounded-[4rem] border-8 border-sky-200 shadow-2xl flex flex-col items-center gap-6">
            ...
            <h2 className="title-font text-4xl md:text-6xl text-sky-600 uppercase">
              ជោគជ័យ!
            </h2>
          </div>
        </div>
