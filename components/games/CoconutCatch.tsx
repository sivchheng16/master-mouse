...
export const CoconutCatch: React.FC<{
  onComplete: () => void;
  count?: number;
}> = ({ onComplete, count = 8 }) => {
    ...
    const loop = setTimeout(() => {
      ...
      const updated = coconuts.map((coconut) => {
        ...
        if (newY > 90 && !coconut.caught) {
          ...
          setTimeout(
            () => setSplashes((s) => s.filter((sp) => sp.id !== splash.id)),
            500,
          );
  ...
  const handleCatch = (coconutId: number) => {
    ...
    setScorePopups((prev) => [
      ...prev,
      { id: popupId, x: coconut.x, y: coconut.y },
    ]);
    setTimeout(
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[25%] pointer-events-none">
        ...
        <div className="absolute bottom-0 w-full h-[80%] bg-[#e6c288] rounded-t-[50%_20%] shadow-[0_-10px_20px_rgba(0,0,0,0.1)]">
          ...
          <div
            ...
            style={{
              backgroundImage:
                "radial-gradient(circle, #b89055 1px, transparent 1px)",
