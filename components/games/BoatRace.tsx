...
export const BoatRace: React.FC<{ onComplete: () => void; count?: number }> = ({
  onComplete,
  count = 3,
}) => {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(180deg, #1e3a5f 0%, #2980b9 30%, #3498db 70%, #5dade2 100%)",
      }}
    >
      <GameHUD
        round={round}
        totalRounds={totalRounds}
        instruction={languageService.t("game.boat_race_instruction")}
        progress={progress}
      />
      ...
      <div className="absolute top-0 left-0 right-0 h-[20%] bg-gradient-to-b from-green-600 to-green-500 flex items-end justify-around overflow-hidden">
        {[...Array(12)].map((_, i) => (
          <div
            key={i}
            className="text-2xl md:text-3xl animate-cheer"
