...
export const KiteFlying: React.FC<{
  onComplete: () => void;
  count?: number;
}> = ({ onComplete, count = 3 }) => {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(180deg, #3aa2f0 0%, #8bcbf9 40%, #cce9ff 100%)",
      }}
    >
      ...
      <div
        className={`absolute left-[25%] right-[25%] h-24 rounded-3xl border-4 transition-all duration-300 z-10 flex items-center justify-between px-4 ${
          isInZone
      ...
      >
        <span
          className={`text-4xl transition-opacity ${isInZone ? "opacity-100" : "opacity-50"}`}
        >
          ✨
        </span>
        <span
      ...
      >
        {/* String Line */}
        <div
          className="absolute top-[80px] left-1/2 w-0.5 h-[1000px] bg-white/60 origin-top -translate-x-1/2"
          style={{ transform: `rotate(${(50 - kiteY) * 0.1}deg)` }}
        />
        ...
        <svg
          ...
          height="160"
          viewBox="0 0 100 140"
          className="overflow-visible animate-kite-sway"
        >
          {/* Frame */}
          <path
            d="M50 0 L90 40 L50 110 L10 40 Z"
            fill="#FF5252"
            stroke="white"
            strokeWidth="2"
          />
        ...
        {isInZone && (
          <div className="absolute -inset-10 pointer-events-none">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="absolute w-full h-0.5 bg-white/40 animate-wind-gust"
                style={{
                  top: `${20 + i * 30}%`,
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[20%] pointer-events-none">
        ...
        {/* Decor */}
        <div className="absolute bottom-4 left-10 text-6xl drop-shadow-md">
          🏡
        </div>
        <div className="absolute bottom-4 right-10 text-6xl drop-shadow-md">
          🌳
        </div>
        <div className="absolute bottom-8 left-1/4 text-4xl drop-shadow-sm">
          🌲
        </div>
      </div>
      ...
      <div className="absolute top-24 left-1/2 -translate-x-1/2 bg-white/20 backdrop-blur-md px-6 py-1 rounded-full border border-white/40">
        <span
          className={`font-mono text-xl ${timeLeft <= 5 ? "text-red-500 font-black animate-ping" : "text-white font-bold"}`}
        >
      ...
      {showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-blue-900/40 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-8 border-blue-200 text-center">
            <h2 className="title-font text-5xl text-blue-600 animate-bounce mb-4">
              អស្ចារ្យ!
            </h2>
            <p className="text-xl font-black text-blue-900 animate-pulse">
              ខ្លែងហោះខ្ពស់ណាស់! 🪁
            </p>
          </div>
        </div>
      ...
      {completed && !showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-blue-900/20 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
          <div className="bg-white/95 p-16 rounded-[3rem] shadow-2xl border-8 border-white text-center transform hover:scale-105 transition-transform">
            <h2 className="title-font text-6xl text-blue-600 animate-tada mb-8">
              ជ័យជំនះ! 🎉
            </h2>
