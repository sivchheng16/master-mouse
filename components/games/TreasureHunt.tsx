...
export const TreasureHunt: React.FC<TreasureHuntProps> = ({ onComplete }) => {
  ...
  return (
    ...
    >
      ...
      {/* Map Decorations */}
      <div className="absolute top-10 left-10 text-7xl opacity-30 rotate-[20deg] pointer-events-none drop-shadow-sm">
        🧭
      </div>
      <div className="absolute bottom-12 right-12 text-7xl opacity-30 -rotate-[15deg] pointer-events-none drop-shadow-sm">
        ⚓
      </div>
      <div className="absolute top-1/2 left-[12%] text-5xl opacity-40 pointer-events-none grayscale">
        🏔️
      </div>
      <div className="absolute bottom-[20%] right-[10%] text-6xl opacity-40 pointer-events-none grayscale">
        🌴
      </div>
      <div className="absolute bottom-10 left-1/4 text-5xl opacity-30 pointer-events-none animate-pulse">
        🌊
      </div>
      <div className="absolute top-24 right-[35%] text-4xl opacity-30 pointer-events-none">
        💀
      </div>
      <div className="absolute top-1/4 right-[10%] text-4xl opacity-20 pointer-events-none">
        ⛵
      </div>
      ...
      <GameHUD
        instruction={
          currentProgress < 0.9 ? "រំកិលម៉ៅតាមគន្លង..." : 'ចុច ២ដងលើ "X"!'
        }
        score={foundCount}
        goal={targets.length}
      />
        ...
        return (
          ...
          >
            <button
              ...
              disabled={isFound}
              className={`relative w-20 h-20 md:w-24 md:h-24 flex items-center justify-center transition-all duration-500 transform ${
                isFound
            ...
            >
              ...
              <div
                className={`relative w-full h-full flex items-center justify-center text-6xl md:text-7xl font-black ${canOpen ? "text-red-600 drop-shadow-[0_0_15px_rgba(239,68,68,0.7)] animate-pulse" : "text-[#8b5e34]/60"}`}
              >
                ✖️
              </div>
              {canOpen && (
            ...
            {isFound && (
              <div className="absolute inset-0 flex flex-col items-center justify-center animate-spring z-20">
                ...
                <div className="bg-[#fdf5e6] backdrop-blur-sm px-5 py-2 rounded-xl border-2 border-[#8b5e34] shadow-2xl -mt-4 transform rotate-2">
                  <span className="text-[12px] md:text-sm font-black text-[#5d4037] whitespace-nowrap uppercase tracking-wide">
                    {t.name}
                  </span>
                </div>
              </div>
            ...
            <div className="flex gap-6">
              {targets.map((t) => (
                <div key={t.id} className="relative">
                  <span
                    className="text-7xl md:text-9xl animate-bounce"
