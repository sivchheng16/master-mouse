...
const CloudSVG: React.FC<{ className?: string; opacity?: number }> = ({
  className,
  opacity = 0.5,
}) => (
  <svg viewBox="0 0 200 100" className={className} style={{ opacity }}>
    <path
      d="M20,60 Q40,30 70,50 Q90,20 130,40 Q160,30 180,60 Q190,80 150,85 L50,85 Q10,80 20,60"
      fill="#FFFFFF"
...
const RealisticPalmTree: React.FC<{ className?: string; flip?: boolean }> = ({
  className,
  flip,
}) => (
  <svg
  ...
  >
    ...
    <path
      d="M10,400 C60,300 -20,150 80,20"
      fill="url(#trunkGrad)"
    ...
    <g transform="translate(80, 20)">
      ...
      <g filter="url(#dropShadow)">
        ...
        <path
          d="M0,0 Q-30,-60 -50,-100 L-45,-100 Q-25,-55 0,0 Z"
          fill="#4CAF50"
        />
...
const WovenHammock: React.FC<{ isHappy: boolean }> = ({ isHappy }) => (
  <svg viewBox="0 0 240 100" className="w-full h-full drop-shadow-xl">
    ...
    <g
      ...
      className={`transition-transform duration-500 ease-in-out ${isHappy ? "scale-110" : "scale-100"}`}
    >
      {/* Head */}
      <circle
        cx="0"
      ...
      <g transform={isHappy ? "translate(0, -1)" : ""}>
        ...
        {/* Mouth */}
        <path
          d={isHappy ? "M-3,5 Q0,8 3,5" : "M-2,5 Q0,6 2,5"}
          fill="none"
          stroke="#5D4037"
          strokeWidth="2"
          strokeLinecap="round"
...
export const HammockSwing: React.FC<{
  onComplete: () => void;
  count?: number;
}> = ({ onComplete, count = 3 }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const inZone =
        swingAngle >= targetZone.min && swingAngle <= targetZone.max;
      if (inZone) {
        setScore((s) => {
          const newScore = s + 1;
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-full overflow-hidden select-none font-fredoka"
    ...
    >
      ...
      <div className="absolute bottom-[30%] left-1/2 -translate-x-1/2">
        <div className="w-48 h-48 bg-yellow-200 rounded-full blur-[60px] opacity-40 animate-pulse" />{" "}
        {/* Large Glow */}
        <div className="absolute top-12 left-12 w-24 h-24 bg-gradient-to-b from-[#FFF59D] to-[#FFB74D] rounded-full shadow-[0_0_50px_rgba(255,167,38,0.8)]" />{" "}
        {/* Core */}
      </div>
      {/* Clouds - White & Subtle */}
      ...
      <div className="absolute bottom-[28%] w-full h-1 bg-white/20 blur-[1px]" />{" "}
      {/* Horizon Line */}
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div
          className="absolute -bottom-10 left-[-80px] w-[300px] h-[600px] z-20 origin-bottom-left"
        ...
        <div
          className="absolute -bottom-10 right-[-80px] w-[300px] h-[600px] z-20 origin-bottom-right"
          style={{ transform: "rotate(-10deg)" }}
        >
        ...
        {/* Swing Mechanism */}
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[100px] w-[280px] h-32 z-10"
          style={ropeStyle}
        >
          {/* Ropes - Visual thickness & rope color */}
          <div
            className="absolute -top-[350px] -left-12 w-1 h-[400px] bg-[#A1887F] origin-bottom shadow-sm"
            style={{ transform: "rotate(-25deg)" }}
          />
          <div
            className="absolute -top-[350px] -right-12 w-1 h-[400px] bg-[#A1887F] origin-bottom shadow-sm"
            style={{ transform: "rotate(25deg)" }}
          />
          ...
          {!hasScrolled && (
            <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 flex flex-col items-center animate-bounce">
              <div className="bg-black/50 backdrop-blur-md text-white text-xs px-3 py-1 rounded-full mb-2 border border-white/20">
                SCROLL TO SWING
              </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-[60%] max-w-sm z-30">
        <div className="relative h-3 bg-black/60 rounded-full backdrop-blur-md border border-white/30 shadow-xl">
          ...
          >
            <div
              className={`absolute inset-0.5 rounded-full ${inZone ? "bg-[#00E676]" : "bg-transparent"}`}
            />
          </div>
        </div>
      </div>
      ...
      {showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/40 backdrop-blur-sm">
          <div className="text-white text-center animate-in fade-in zoom-in duration-500">
            <h2 className="text-6xl font-bold mb-2 drop-shadow-lg text-[#FFD54F]">
              អស្ចារ្យ!
            </h2>
      ...
      {score >= targetScore && round === totalRounds && !showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center z-50 bg-black/60 backdrop-blur-md">
          <div className="text-white text-center animate-in slide-in-from-bottom duration-700">
            <h2 className="text-5xl font-bold mb-4 drop-shadow-[0_4px_10px_rgba(0,0,0,0.5)]">
              សម្រាកបានល្អ!
            </h2>
            <button
              onClick={onComplete}
              className="px-8 py-3 bg-[#4CAF50] hover:bg-[#43A047] text-white font-bold rounded-full shadow-lg transition-all transform hover:scale-105"
            >
              បន្តទៅមុខទៀត
            </button>
          </div>
