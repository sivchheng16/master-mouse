...
export const FruitChop: React.FC<{
  onComplete: () => void;
  count?: number;
}> = ({ onComplete, count = 10 }) => {
  ...
  useEffect(() => {
    ...
    const updateGame = () => {
      ...
      // Spawn Logic
      if (
        now - lastSpawnRef.current > 2500 - round * 400 &&
      ...
      if (scoreRef.current >= targetScore) {
        // GUARD: If we already triggered completion (e.g. during the 2s delay), stop here.
        // We can check if we are already showing the level up or if we are blocked.
        // However, simply returning here prevents further updates.
        // But we must ensure handleRoundComplete is called EXACTLY ONCE.
        // The issue is this loop runs every frame.

        // Better approach: Check if we are already in "level up" state or waiting.
        // But showLevelUp is state, might not update immediately in this ref-heavy loop.
        // Let's use a flag or just rely on the fact that we stop calling requestAnimationFrame?

        // If we return here, the loop stops (we don't call requestAnimationFrame below).
        // AND we call handleRoundComplete.
        // We need to make sure we don't call it repeatedly if this frame runs multiple times before loop stops?
        // Actually, if we return, we don't request next frame. So correctness relies on not requesting next frame.

        if (round < totalRounds) {
          // Check if we haven't already scheduled it?
          // The problem is if score jumps > targetScore, we enter here.
          // We call handleRoundComplete.
          // We return. Loop ends.
          // So it should be fine, UNLESS handleRoundComplete has the delay which we added.
          // We added the delay in handleRoundComplete.
          // BUT, did we safeguard handleRoundComplete itself?
          // The previous issue was the loop KEEPS RUNNING.
          // By returning here, we stop the loop.
  ...
  const spawnFruit = () => {
    ...
    const dirX =
      startX < 50 ? 0.1 + Math.random() * 0.2 : -(0.1 + Math.random() * 0.2);
    ...
    setBladePath((path) => {
      ...
      setFruits((currentFruits) => {
        ...
        const nextFruits = currentFruits.map((fruit) => {
          ...
          const ratio =
            (boardRef.current?.offsetHeight || 1) /
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            boxShadow:
              "0 0 10px 4px #22d3ee, 0 0 20px 10px rgba(6, 182, 212, 0.5)",
            opacity: 0,
      ...
      {showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-8 border-yellow-400 text-center">
            <h2 className="title-font text-6xl text-amber-600 animate-bounce mb-4">
              អស្ចារ្យ!
            </h2>
      ...
      {bombExploded && (
        <div className="absolute inset-0 flex items-center justify-center bg-red-900/80 backdrop-blur-sm z-[100] animate-in fade-in duration-300">
          <div className="text-center transform scale-150">
            ...
            <h2 className="title-font text-6xl text-white mt-4 drop-shadow-lg animate-pulse">
              ប្រយ័ត្នគ្រាប់បែក!
            </h2>
          </div>
        </div>
      ...
      {round === totalRounds && score >= targetScore && !showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/80 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
          <div className="bg-gray-900 p-12 rounded-[3.5rem] shadow-2xl border-8 border-amber-400 text-center text-white">
            <h2 className="title-font text-6xl text-yellow-400 animate-bounce mb-6">
              ជ័យជំនះ! 🏆
            </h2>
