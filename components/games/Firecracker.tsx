...
export const Firecracker: React.FC<{ onComplete: () => void; count?: number }> = ({ onComplete, count = 5 }) => {
        ...
        const interval = setInterval(() => {
            setFirecrackers(prev => {
                ...
                const nextFirecrackers = prev.map(fc => {
                    ...
                    const currentSpeed = (fc.fuseProgress >= 70 && fc.fuseProgress < 95)
                        ? 0.25
                        : fc.speed;
    ...
    }, [exploded, missedCount, currentCount, round, totalRounds, gameOver, showLevelUp, onComplete]);
    ...
    return (
        <div className="relative w-full h-full overflow-hidden select-none bg-gradient-to-b from-indigo-900 via-purple-900 to-black">
            ...
            <div className="absolute top-4 left-4 flex gap-2 z-10">
                {[...Array(3)].map((_, i) => (
                    <span key={i} className={`text-3xl ${i < (3 - missedCount) ? 'grayscale-0' : 'grayscale opacity-30'}`}>
                        ❤️
                    </span>
                ))}
            </div>
            ...
            {firecrackers.map(fc => (
                <div
                    ...
                    onClick={() => !fc.exploded && !fc.missed && fc.startDelay <= 0 && handleClick(fc.id)}
                    onMouseEnter={() => !fc.exploded && !fc.missed && fc.startDelay <= 0 && audioService.playHover()}
                >
                    {/* Firecracker body */}
                    <div className={`relative ${fc.fuseProgress >= 70 && !fc.exploded && !fc.missed ? 'animate-shake' : ''}`}>
                        {fc.exploded ? (
                            // Explosion effect
            ...
            {/* Completion */}
            {round === totalRounds && exploded === currentCount && !showLevelUp && !gameOver && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
                    <div className="bg-gradient-to-b from-yellow-400 to-orange-500 p-12 rounded-[3.5rem] shadow-2xl border-8 border-yellow-300 text-center">
                        <h2 className="title-font text-5xl text-white animate-bounce mb-6 drop-shadow-lg">កាំជ្រួចស្រស់ស្អាតណាស់! 🎉</h2>
            ...
            {gameOver && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-md z-[100] animate-in fade-in zoom-in duration-500">
                    <div className="bg-gradient-to-b from-red-500 to-red-700 p-12 rounded-[3.5rem] shadow-2xl border-8 border-red-400 text-center">
                        <h2 className="title-font text-4xl text-white animate-bounce mb-4 drop-shadow-lg">ហ្គេមចាញ់ហើយ!</h2>
                        <p className="text-lg font-black text-red-100 mb-6">អ្នកបានខកខាន ៣ ដង </p>
                        <button
                            onClick={restartGame}
