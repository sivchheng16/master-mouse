...
export const TukTukDriver: React.FC<{
  onComplete: () => void;
  count?: number;
}> = ({ onComplete, count = 3 }) => {
  ...
  const initRound = (r: number) => {
    ...
    const newObstacles: Obstacle[] = Array.from({ length: numObstacles }).map(
      (_, i) => ({
        id: i,
        x: 30 + Math.random() * 40,
        y: 25 + Math.random() * 50,
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(180deg, #87CEEB 0%, #87CEEB 15%, #808080 15%, #808080 100%)",
      }}
    >
      {/* Buildings in background */}
      <div className="absolute top-0 left-0 right-0 h-[15%] flex items-end justify-around pointer-events-none">
        {["🏢", "🏬", "🏪", "🏠", "🏨", "🏦", "🏪"].map((building, i) => (
          <div key={i} className="text-4xl md:text-5xl opacity-80">
            {building}
          </div>
        ))}
      </div>
      ...
      {completed && round === totalRounds && !showLevelUp && (
        <div className="absolute inset-0 flex items-center justify-center bg-amber-900/20 backdrop-blur-md z-50 animate-in fade-in zoom-in duration-500">
          <div className="bg-white/90 p-12 rounded-[3.5rem] shadow-2xl border-8 border-white text-center">
            <h2 className="title-font text-5xl text-amber-600 animate-bounce mb-6">
              អ្នកបើកតុកតុកពូកែ! 🎉
            </h2>
