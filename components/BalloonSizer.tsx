...
export const BalloonSizer: React.FC<BalloonSizerProps> = ({ onComplete, tolerance = 12 }) => {
  ...
  return (
    ...
    >
      <div className="absolute top-10 text-center z-20 w-full px-4">
        <div className="inline-block bg-white/60 backdrop-blur-md px-8 py-3 rounded-3xl border-2 border-sky-200 shadow-xl">
          <h2 className="text-xl md:text-2xl font-black text-sky-900 leading-tight uppercase tracking-tight">
            ប្រើកង់ម៉ៅ (SCROLL) ដើម្បីតម្រូវទំហំ!
          </h2>
        </div>
      </div>
      ...
      <div className="relative flex items-center justify-center pointer-events-none">
        ...
        <div 
          className={`absolute border-[6px] border-dashed rounded-full transition-all duration-300 flex items-center justify-center ${
            isPerfect ? 'border-emerald-500 scale-105 opacity-100' : isNear ? 'border-sky-400 animate-pulse' : 'border-sky-300/40'
          }`}
        ...
        >
          <div 
            className={`w-full h-full rounded-full transition-colors duration-300 shadow-2xl relative flex items-center justify-center ${
              isPerfect ? 'bg-emerald-500' : isNear ? 'bg-orange-400' : 'bg-red-500'
            }`}
            style={{ 
